...
var verify_is_token_valid_controller_exports = {};
__export(verify_is_token_valid_controller_exports, {
  VerifyTokenIsValid: () => VerifyTokenIsValid
});
module.exports = __toCommonJS(verify_is_token_valid_controller_exports);
...
async function VerifyTokenIsValid(request, reply) {
  try {
    const tokenSchema = import_zod2.z.object({
      token: import_zod2.z.coerce.string()
    });
    const {
      token
