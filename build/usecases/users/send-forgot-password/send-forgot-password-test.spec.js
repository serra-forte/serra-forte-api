...
// src/usecases/users/send-forgot-password/send-forgot-password-test.spec.ts
...
var SendForgotPasswordUseCase = class {
  constructor(usersRepository, usersTokensRepository, dayjsDateProvider2, sendMailProvider2) {
...
var stu;
(0, import_vitest.describe)("Send forgot password user (unit)", () => {
  ...
  (0, import_vitest.test)("Should be able to send forgot password", async () => {
    await stu.execute({
      email: "user1-test@email.com"
    });
    const userToken = await usersTokensRepositoryInMemory.findByUserId("id-user-1");
    (0, import_vitest.expect)(userToken.token).toEqual(import_vitest.expect.any(String));
  ...
  (0, import_vitest.test)("Should not be able to send forgot password with wrong e-mail", async () => {
