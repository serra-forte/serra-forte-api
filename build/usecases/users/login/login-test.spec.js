...
// src/usecases/users/login/login-test.spec.ts
...
var stu;
(0, import_vitest.describe)("Login user (unit)", () => {
  ...
  (0, import_vitest.test)("Should be able to login a account", async () => {
    ...
    (0, import_vitest.expect)(accessToken).toEqual(import_vitest.expect.any(String));
    (0, import_vitest.expect)(refreshToken).toEqual(import_vitest.expect.any(String));
    const usersToken = await usersTokensRepositoryInMemory.findByToken(refreshToken);
    (0, import_vitest.expect)(usersToken?.token).toEqual(refreshToken);
  });
  (0, import_vitest.test)("should not be able to login with wrong password", async () => {
    await (0, import_vitest.expect)(
      () => stu.execute({
        email: "email@test.com",
        password: "12345666"
      })
    ).rejects.toEqual(new AppError("Usu\xE1rio ou senha incorretos", 401));
  });
  (0, import_vitest.test)("should not be able to login with wrong email", async () => {
    await (0, import_vitest.expect)(
      () => stu.execute({
