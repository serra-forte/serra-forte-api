...
// src/providers/PaymentProvider/implementations/provider-paypal-payment.ts
var provider_paypal_payment_exports = {};
__export(provider_paypal_payment_exports, {
  PaypalProvider: () => PaypalProvider
});
module.exports = __toCommonJS(provider_paypal_payment_exports);
...
var PaypalProvider = class {
  async authorizeOrderPayment(id, payment, access_token) {
    try {
      const responseCreatePayment = await axios_default.post(`${env.PAYPAL_API_URL}/v2/checkout/orders/${id}/confirm-payment-source`, payment, {
        headers: {
  ...
  async captureOrdePayment(payment, id, access_token) {
    try {
      const responseCreatePayment = await axios_default.post(`${env.PAYPAL_API_URL}/v2/checkout/orders/${id}/capture`, payment, {
        headers: {
  ...
  async createOrderPayment(payment, access_token) {
    try {
      const responseCreatePayment = await axios_default.post(`${env.PAYPAL_API_URL}/v2/checkout/orders`, payment, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${access_token}`,
          "PayPal-Request-Id": "556268e0-43df-424b-9bbe-ea9aa3cf2402"
  ...
  async generateAccessToken() {
    try {
      const responseGenerateAccessToken = await axios_default.post(
        `${env.PAYPAL_API_URL}/v1/oauth2/token`,
        "grant_type=client_credentials",
        // Dados devem ser enviados como uma string
        {
          headers: {
