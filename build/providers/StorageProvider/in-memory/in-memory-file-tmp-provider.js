...
// src/providers/StorageProvider/in-memory/in-memory-file-tmp-provider.ts
var in_memory_file_tmp_provider_exports = {};
__export(in_memory_file_tmp_provider_exports, {
  InMemoryFileTMPProvider: () => InMemoryFileTMPProvider
});
module.exports = __toCommonJS(in_memory_file_tmp_provider_exports);
...
var InMemoryFileTMPProvider = class {
  deleteFileTmp(fileName, folderPath) {
    try {
      ...
      if (!import_fs.default.existsSync(`${path}/${folderPath}/${fileName}`)) {
        console.log("Arquivo n\xE3o existe na pasta tmp");
        return null;
      }
